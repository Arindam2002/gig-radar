...
from jobscout import cards, concepts, db, graph
...
def test_cli_says_so_when_it_does_not_know_the_command(cli, capsys):
    assert concepts._cli(["concepts", "summarise"]) == 0
    assert "Use: check | order | sheet" in capsys.readouterr().out
...
# ── C1: definitions and the sheet ───────────────────────────────────
#
# The fixture folder carries no decks - it was built to exercise the graph,
# and a deck committed next to it would be a second thing to keep in step.
# These tests write the deck they need into the tmp_path copy, so what each
# assertion depends on is visible in the test that makes it.

# One definitional card and one that is not. The second mentions the same
# concept on purpose: it is the shape that made "definitional card only" a
# rule, straight off the real SQL deck.
ALPHA_DECK = (
    "# Alpha - cards\n\n"
    "- Q: What is a covering index?\n"
    "  A: One where every column the query needs is in the index itself.\n"
    "\n"
    "- Q: What three things do you read first in an execution plan?\n"
    "  A: The heaviest operator, the estimate/actual ratio, and any Sort.\n"
    "\n"
    "- Q: Define change tracking.\n"
    "  A: The snapshot the context keeps so it can work out what to write.\n")


@pytest.fixture()
def decked(study):
    (study / "cards").mkdir(exist_ok=True)
    (study / "cards" / "concepts-alpha.md").write_text(ALPHA_DECK)
    return study


def sheet_of(study, conn=None, **kw):
    return concepts.sheet(concepts.build(study, conn), study, conn, **kw)


def test_c1_a_definitional_card_defines_and_nothing_else_does(decked):
    deck = cards.load(decked, "concepts-alpha")
    assert concepts.definition_for("covering index", deck).startswith(
        "One where every column")
    assert concepts.definition_for("change tracking", deck).startswith(
        "The snapshot")
    # named by a card that opens "What three things", so it is not a
    # definition however often the deck says the words
    assert concepts.definition_for("execution plan", deck) is None
    assert concepts.definition_for("connection pool", deck) is None
    assert concepts.definition_for("", deck) is None
    assert concepts.definition_for("covering index", []) is None


def test_c1_a_name_is_matched_whole_and_across_the_separator():
    deck = [{"q": "What is KV-cache paging?", "a": "Blocks, not one slab."},
            {"q": "What is indexing?", "a": "Not the answer for 'index'."}]
    for spelling in ("KV cache paging", "kv-cache paging", "KV_cache paging"):
        assert concepts.definition_for(spelling, deck) == "Blocks, not one slab."
    assert concepts.definition_for("index", deck) is None      # whole words


def test_c1_the_sheet_has_the_header_the_tracks_and_the_topics(decked):
    text = sheet_of(decked)
    assert text.startswith("# Concepts\n")
    assert ("_10 topics, 46 concepts, 2 shared across topics. Generated by "
            "`python -m jobscout.concepts sheet`; do not edit._") in text
    assert "\n## backend\n" in text and "\n## dsa\n" in text
    assert "resume-concepts-drill" not in text                 # drills are not listed
    assert ("### [Alpha, a synthetic backend topic]"
            "(topics/concepts-alpha.md) · 2 questions · 1 round · not studied"
            ) in text
    assert ("### [Beta, the middle of the chain](topics/concepts-beta.md) · "
            "0 questions · 0 rounds · not studied") in text


def test_c1_a_concept_shows_its_definition_or_the_gap(decked):
    text = sheet_of(decked)
    assert ("**covering index** — One where every column the query needs is "
            "in the index itself.") in text
    assert "**execution plan** — (no card yet)" in text
    assert "**write amplification** — (no card yet)" in text
    # beta spells alpha's concept differently and has no deck of its own; the
    # definition follows the concept, not the spelling
    assert "**change tracker** — The snapshot" in text


def test_c1_every_concept_is_anchored_once(decked):
    text = sheet_of(decked)
    assert '- <a id="cover-index"></a>**covering index**' in text
    ids = re.findall(r'<a id="([^"]+)"></a>', text)
    assert len(ids) == len(set(ids))                           # unique on the page
    assert set(ids) == {concepts.anchor_for(k)
                        for k in concepts.build(decked)["concepts"]}
    assert concepts.anchor_for(("concept", "shar")) == "concept-shar"
    assert concepts.anchor_for("c:concept shar") == "concept-shar"


def test_c1_shared_concepts_and_the_study_order_close_the_sheet(decked):
    text = sheet_of(decked)
    shared = text.split("## Shared concepts", 1)[1]
    assert "**shared concept** — concepts-alpha, concepts-beta" in shared
    assert "**change tracking** — concepts-alpha, concepts-beta" in shared
    order = text.split("## Study order", 1)[1]
    assert "1. concepts-alpha (ready)" in order
    assert order.index("concepts-alpha") < order.index("concepts-gamma")


def test_c1_a_topic_with_no_concepts_says_so(decked):
    assert "- (no concepts named yet)" in sheet_of(decked)


def test_c1_public_mode_strips_the_lot_and_keeps_the_export_set(decked):
    text = sheet_of(decked, public=True,
                    export_set={"concepts-alpha", "concepts-beta"})
    # five names each, two of which are the same concept twice over
    assert "_2 topics, 8 concepts, 2 shared across topics. " \
           "Generated from the published topics._" in text
    assert "[[concepts-alpha|Alpha, a synthetic backend topic]]" in text
    assert "[[concepts-beta|Beta, the middle of the chain]]" in text
    assert "concepts-gamma" not in text and "sliding window" not in text
    assert "](topics/" not in text                             # no md links
    assert "<a id=" not in text                                # no anchors
    assert "studied" not in text                               # nor "not studied"
    assert "question" not in text and "round" not in text
    assert "Study order" not in text
    assert "**covering index** — One where every column" in text
    assert "**shared concept** — [[concepts-alpha|" in text


def test_c1_the_cli_writes_the_sheet_where_new_names_looks_for_it(
        cli, study, capsys):
    assert concepts._cli(["concepts", "sheet"]) == 0
    written = capsys.readouterr().out.strip()
    path = concepts.sheet_path(study)
    assert written == str(path) and path.is_file()
    # and the day's diff closes: nothing is new once the sheet knows it
    assert concepts.new_names(concepts.build(study), path) == []


def test_c1_write_sheet_is_idempotent(study):
    first = concepts.write_sheet(concepts.build(study), study)
    text = first.read_text()
    assert concepts.write_sheet(concepts.build(study), study).read_text() == text


# ── C1b: the real decks, when the real folder is there ──────────────

REAL_STUDY = Path.home() / "Personal" / "job-scout" / "study"
REAL_DECK = REAL_STUDY / "cards" / "sql-indexing-query-performance.md"


@pytest.mark.skipif(not REAL_DECK.is_file(),
                    reason="the real study folder is not on this machine")
def test_c1b_the_real_sql_deck_defines_one_of_these_and_not_the_other():
    """The pair that made the rule. Both concepts are all over the deck; only
    one of them has a card that says what it IS."""
    deck = cards.parse(REAL_DECK.read_text())
    assert deck                                                # it parsed
    assert concepts.definition_for("execution plan", deck) is None
    covering = concepts.definition_for("covering index", deck)
    assert isinstance(covering, str) and covering.strip()
